...
export const useProductActions = () => {
  ...
  const [localQuery, setLocalQuery] = useState<string>(ProductsState.searchQuery || '');
  ...
  const transformApiProduct = useCallback((product: Product) => {
    ...
    return {
      ...
      rating: product.reviews.reduce((sum, review) => sum + parseFloat(review.rating), 0) / product.reviews.length,
      reviewCount: product.reviews.length,
      isNew: new Date(product.createdDate).getTime() > new Date().getTime() - 1000 * 60 * 60 * 24 * 30,
      discount: primaryVariant?.amount_discount_price
        ? Math.round(((primaryVariant.price - parseFloat(primaryVariant.amount_discount_price)) / primaryVariant.price) * 100)
        : 0,
  ...
  const calculatePriceRange = useCallback(() => {
    ...
    const prices = transformedProducts.map((product) => product.price).filter((price) => price !== null && price !== undefined);
  ...
  const setSelectedCategoryAction = useCallback((category: string | null) => {
  ...
  const setProductsErrorAction = useCallback((error: string | null) => {
    dispatch(setProductsError(error));
  }, [dispatch]);

  const setSelectedPriceRangeAction = useCallback((priceRange: [number, number]) => {
    setPriceRange(priceRange);
  }, [setPriceRange]);

  const setSelectedSortBy = useCallback((sortBy: 'rating' | 'newest' | 'oldest' | 'price-low' | 'price-high' | 'price-asc' | 'price-desc') => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    const query = (localQuery || ProductsState.searchQuery).toLowerCase().trim();
    ...
    return transformedProducts.filter((product) => {
      ...
      const matchesCategory = !ProductsState.selectedCategory ||
        productCategoryIds.includes(Number(ProductsState.selectedCategory));

      const matchesPrice = !priceRange || (product.min_price >= priceRange[0] && product.max_price <= priceRange[1]);

      return matchesSearch && matchesCategory && matchesPrice;
    });
  }, [transformedProducts, localQuery, ProductsState.selectedCategory, priceRange, ProductsState.searchQuery]);
  ...
  const computedState = useMemo(() => ({
    ...
    isLoading: isLoading,
  }), [transformedProducts, ProductsState, isLoading]);
